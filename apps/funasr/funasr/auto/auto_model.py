                    ...
                    if k.startswith("timestamp"):
                        ...
                        for t in restored_data[j][k]:
                            if isinstance(t, dict):
                                ...
                                normalized_timestamps.append(t)
                            elif (
                                isinstance(t, (list, tuple))
                                and len(t) >= 3
                                and isinstance(t[0], str)
                            ):
                                # SenseVoice remote code emits [token, start_sec, end_sec].
                                # Normalize it to FunASR's millisecond timestamp shape and
                                # retain the token surface for punctuation alignment.
                                result.setdefault("words", []).append(t[0])
                                normalized = [
                                    round(float(t[1]) * 1000) + int(vadsegments[j][0]),
                                    round(float(t[2]) * 1000) + int(vadsegments[j][0]),
                                ]
                                ...
                                normalized = list(t)
                                normalized[0] = int(normalized[0]) + int(vadsegments[j][0])
                                normalized[1] = int(normalized[1]) + int(vadsegments[j][0])
                                if isinstance(t, list):
                                    t[:] = normalized
                                normalized_timestamps.append(normalized)
                        result[k].extend(normalized_timestamps)
